...
SRC_URI = "git://github.com/tuxbox-neutrino/linux-kernel-cst.git;protocol=https;branch=cst_${PV} \
           file://0001-dtc-lexer-extern-yylloc.patch \
           file://0002-arm-force-soft-float.patch \
           file://0003-compiler-gcc6-compat.patch \
           file://0004-disable-arm-virt-ext-default.patch \
           file://0005-disable-eabi-float-helpers.patch"
...
do_configure() {
    ...
    cp ${WORKDIR}/${CST_DEFCONFIG} ${B}/.config
    oe_runmake -C ${S} O=${B} olddefconfig
    if grep -q '^CONFIG_ARM_VIRT_EXT=' ${B}/.config; then
        sed -i 's/^CONFIG_ARM_VIRT_EXT=.*/# CONFIG_ARM_VIRT_EXT is not set/' ${B}/.config
    else
        echo '# CONFIG_ARM_VIRT_EXT is not set' >> ${B}/.config
    fi
    oe_runmake -C ${S} O=${B} olddefconfig
}
